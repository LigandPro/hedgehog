...
- **How it works**: Scores synthetic complexity from Morgan fingerprints using the published SCScore neural model. When `scscore` is included in `enabled_scores`, the score is computed and is filtered only when `score_filters.sc_score` thresholds are present.
...
2. With `aizynthfinder_charge_mode: both`, Hedgehog searches the original SMILES and a stereochemistry-preserving neutralized form when it differs.
3. AiZynthFinder runs the variants as one normal multiprocessing batch against its configured policies and stocks.
4. Variant results are merged back onto the original input row, preferring a solved route from either charge representation.

With `aizynthfinder_retry_unsolved: true`, Hedgehog first runs the normal search budget, removes every molecule solved by any charge representation, and reruns only the remaining representations with `aizynthfinder_retry_time_limit` and `aizynthfinder_retry_iteration_limit`. Pass-1, retry, and merged variant JSON files are retained separately.

The shipped search profile uses the limits declared in `config_synthesis.yml`; each representation receives that search budget.

If `filter_solved_only: true`, only molecules for which AiZynthFinder found at least one valid route are kept.
...
## Target-Aware Thresholds

Synthesis is skipped during target alignment. The candidate run uses the source synthesis config unchanged, including SA/RA/SYBA thresholds and retrosynthesis settings.

## Filtering Logic

Score thresholds from `sa_score_*`, `syba_score_*`, `ra_score_*`, and any configured `score_filters` entries are always applied when those score columns exist.
There is no separate runtime switch that disables score filtering while scores are computed.
...
Optional scorers (`sync`, `fsscore`, `gasa`, `nonpher`, …) need their dependencies. With `--auto-install` / `HEDGEHOG_AUTO_INSTALL=1` (and `sync_auto_install` for SYNC), missing runtimes are bootstrapped when possible; otherwise the score column is `NaN` and synthesis continues.
...
| `retrosynthesis_results.json` | Best result per original input molecule, including its route tree |
| `retrosynthesis_variants.json` | Raw results for preserved and neutralized search representations before union |

`synthesis_extended.csv` retains literal AiZynthFinder diagnostics for every
target. These include `retrosynthesis_status`, `retrosynthesis_reason`, observed
search time and iterations, best-route depth, configured limits, whether each
limit was reached, route and node counts, precursor stock counts, and the exact
`precursors_not_in_stock` string. The reason is assembled mechanically from the
saved AiZynthFinder fields; it reports observed stopping conditions and the best
saved route without claiming an unobserved chemical cause.
