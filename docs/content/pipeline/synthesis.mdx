...
The synthesis stage evaluates whether generated molecules can be practically synthesized. It computes four independent accessibility scores and optionally runs full retrosynthetic route analysis using AiZynthFinder.
...
Setting `run_retrosynthesis: false` skips AiZynthFinder entirely and only computes the score columns above, which is significantly faster.
...
4. SYNC Score is within `[sync_score_min, sync_score_max]` when `run_sync: true`
...
| File | Description |
|------|-------------|
| `synthesis_scores.csv` | SA Score, RA Score, SYBA Score, and SYNC Score for all input molecules |
