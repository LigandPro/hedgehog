...
|---|-------|---------|-------------------|
| 1 | [Preprocessing](/pipeline/mol-prep.mdx) | Standardize molecules; remove salts/fragments; preserve charge and stereo by default | 70--95% |
| 2 | [Descriptors](/pipeline/descriptors.mdx) | Compute physicochemical descriptors and filter by threshold ranges | 25--40% |
| 3 | [Structural Filters](/pipeline/structural-filters.mdx) | Calculate liability diagnostics; enforce the configured hard gate (default: PAINS + NIBR + Lilly160 + MolGraph + protecting groups + undefined stereo) | 15--30% |
| 4 | [Synthesis Feasibility](/pipeline/synthesis.mdx) | Evaluate synthetic accessibility scores and optional AiZynthFinder retrosynthesis | 35--60% |
| 5 | [Molecular Docking](/pipeline/docking.mdx) | Dock surviving molecules into the target protein binding site | 50--75% |
| 6 | [Three-Dimensional Filters](/pipeline/docking-filters.mdx) | Filter poses by search-box containment, clashes, interactions, and conformer deviation | 35--60% |
...
Any stage can be requested with `--stage <name>`, which is useful for debugging or re-running a single step. Only the selected stages run; Mol Prep is not forced automatically. Include `--stage mol_prep` if you want standardization first. All configuration files are snapshotted into a `configs/` directory at the start of each run, and a `RUN_INFO.md` is generated at completion.
...
| `descriptors` | Molecule table | preprocessing output, sampled input | `stages/02_descriptors_initial/metrics/` and `stages/02_descriptors_initial/filtered_molecules.csv` |
| `struct_filters` | Descriptor or molecule table | Descriptors output, then preprocessing or sample fallback | `stages/03_structural_filters_post/filtered_molecules.csv` |
