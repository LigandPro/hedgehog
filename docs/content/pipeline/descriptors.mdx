...
This stage runs twice in the pipeline: once as Stage 1 (initial filtering) and once as the final stage (recompute descriptors on the surviving molecule set). Filtering behavior is controlled by `filter_data` in `config_descriptors.yml` and applies to both runs unless you disable it.
...
|-----------|------------|-------------|-------------|-------------|
| Allowed Characters | `allowed_chars` | Set of allowed atom types in SMILES | C,N,S,O,F,Cl,Br,H | -- |
...
| Nitrogen Fraction | `fN_atoms` | Fraction of atoms that are nitrogen | 0 | 1 |
...
| Molecular Weight | `molWt` | Daltons | 200 | 500 |
| logP | `logP` | Octanol-water partition coefficient (Wildman-Crippen) | -0.4 | 5.6 |
| clogP | `clogP` | Calculated logP | -100 | 100 |
| Sw | `sw` | Water solubility estimate | -100 | 100 |
...
| Rotatable Bonds | `n_rot_bonds` | Freely rotatable bonds | 2 | 8 |
| H-Bond Donors | `hbd` | Hydrogen bond donor count | 0 | 5 |
| H-Bond Acceptors | `hba` | Hydrogen bond acceptor count | 0 | 10 |
| Fraction sp3 | `fsp3` | Fraction of sp3-hybridized carbons | 0.15 | 0.8 |
| TPSA | `tpsa` | Topological Polar Surface Area (A^2) | 0 | 140 |
...
| MCE-18 | `mce18` | Molecular complexity estimate (J. Med. Chem. 2019) | 20 | -- |
...
```yaml
# Example: only keep molecules with MW 200-500 and logP -0.4 to 5.6
borders:
  molWt_min: 200
  molWt_max: 500
  logP_min: -0.4
...
| `metrics/skipped_molecules.csv` | Molecules that failed SMILES parsing |
