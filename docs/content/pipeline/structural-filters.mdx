...
With target alignment enabled, Hedgehog recalculates the ring cutoff, both stereocenter limits, all three halogen limits, and the symmetry threshold from the requested retained target percentile. It then recomputes those four pass masks and decides which complete structural rules can remain enabled without dropping below the combined retention target. Execution-only settings such as schedulers are not percentile parameters and remain unchanged.
