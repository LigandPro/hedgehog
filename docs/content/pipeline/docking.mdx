...
Matcha can run through the legacy `LigandPro/Matcha` CLI or the
`LigandPro/docking` screening backend. The screening backend batches ETKDG
conformer generation through nvMolKit and enforces a per-ligand timeout, which
avoids unbounded serial RDKit embedding attempts.
