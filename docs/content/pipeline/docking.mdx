...
Set it to `true` only when you explicitly want to run the configured external
...
For production runs, prefer a controlled Matcha checkout or an explicitly pinned
setup path instead of relying on dynamically selected development code.
