...
The docking stage prepares ligands and runs molecular docking simulations against a target protein structure. HEDGEHOG supports three docking engines (SMINA, GNINA, and Matcha) and two ligand preparation methods.
...
```yaml
tools: gnina    # Options: gnina, smina, matcha, both
```

When `tools: both`, the pipeline runs SMINA and GNINA sequentially. Matcha is enabled explicitly with `tools: matcha` or a comma-separated YAML value such as `gnina,matcha`.
...
You can request parallel ligand preparation in `config_docking.yml`:

```yaml
prep_njobs: auto
```

This setting is applied only when an external ligand preparation tool is configured and supports parallel execution.
...
```yaml
run: true
tools: gnina                              # gnina, smina, matcha, or both
receptor_pdb: src/hedgehog/configs/examples/7EW9_apo.pdb
