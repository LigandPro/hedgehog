...
```yaml
# my_config.yml
generated_mols_path: data/test/generated_mols.csv
...
Note: `src/hedgehog/configs/config.yml` points `pains_file_path` and `mcf_file_path` at `data/` by default, but this repository vendors `wehi_pains.csv` and `mcf.csv` at `src/hedgehog/vendor/moleval/metrics/wehi_pains.csv` and `src/hedgehog/vendor/moleval/metrics/mcf.csv`. Point `pains_file_path` / `mcf_file_path` to those paths (as shown above) or copy the files into `data/`.
