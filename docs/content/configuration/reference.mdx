...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `backend` | string | `matcha_cli` | Use `docking` for the LigandPro/docking screening API or `matcha_cli` for the legacy Matcha CLI |
| `checkout_dir` | string | `modules/matcha_remote` | Managed Matcha checkout directory populated from GitHub |
| `update_checkout` | bool | backend-dependent | Update a managed legacy checkout; defaults to `false` for a caller-owned docking repository |
...
The `docking` backend additionally requires `training_config`,
`checkpoint_root`, and `checkpoint_run`. It uses nvMolKit to generate all
ETKDG conformers for a ligand as one GPU batch with a real timeout. Hedgehog
converts the resulting positive `cnn_affinity` to
`minimizedAffinity = -cnn_affinity` so existing docking thresholds retain
their lower-is-better meaning.
