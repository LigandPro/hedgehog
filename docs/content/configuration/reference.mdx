...
These constraints add Inventum-inspired topology-aware caps on typed atom classes,
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `enabled` | bool | `true` | Enable or disable all structural constraints |
| `type_limits` | dict[string, int] | _(see YAML below)_ | Maximum allowed count for each typed atom class alias (`.=O`, `Car`, `Nd+`, etc.) |
| `element_limits.N` | int | `6` | Maximum allowed nitrogen atom count |
| `element_limits.O` | int | `4` | Maximum allowed oxygen atom count |
| `element_limits.S` | int | `1` | Maximum allowed sulfur atom count |
| `max_n_or_o_atoms` | int | `10` | Maximum allowed combined count of nitrogen and oxygen atoms |
| `max_small_rings_3_4` | int | `0` | Maximum allowed count of 3- or 4-membered rings |
| `max_acyclic_chain_length` | int | `4` | Maximum allowed longest acyclic chain length |
