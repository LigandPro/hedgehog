...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `allowed_chars` | list[string] | `[C, N, S, O, F, Cl, Br, H]` | Allowed chemical elements in molecules |
...
| `fN_atoms_min` / `fN_atoms_max` | float | `0` / `1` | Fraction of nitrogen atoms range |
...
| `molWt_min` / `molWt_max` | float | `200` / `500` | Molecular weight range (Da) |
| `logP_min` / `logP_max` | float | `-0.4` / `5.6` | Crippen logP range |
| `clogP_min` / `clogP_max` | float | `-100` / `100` | Calculated logP range |
| `sw_min` / `sw_max` | float | `-100` / `100` | Sw (water solubility estimate) range |
...
| `n_rot_bonds_min` / `n_rot_bonds_max` | int | `2` / `8` | Rotatable bond count range |
| `hbd_min` / `hbd_max` | int | `0` / `5` | Hydrogen bond donor count range |
| `hba_min` / `hba_max` | int | `0` / `10` | Hydrogen bond acceptor count range |
| `fsp3_min` / `fsp3_max` | float | `0.15` / `0.8` | Fraction of sp3 carbons range |
| `tpsa_min` / `tpsa_max` | float | `0` / `140` | Topological polar surface area range (A^2) |
...
| `run_before_descriptors` | bool | `true` | Run structural filters before descriptor calculation (pre-filter) |
...
Controls generative evaluation metrics computed during report generation. These metrics assess diversity, scaffold coverage, drug-likeness, and chemical group presence across pipeline stages.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `rules_compliance` | bool | `true` | Compute per-stage pass rates for drug-likeness rule sets |
| `rules_sets` | list[string] | `[rule_of_five, rule_of_veber, rule_of_generative_design]` | Rule sets to evaluate: Lipinski's Rule of Five, Veber rules, generative design rules |

### Chemical Group Presence

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `chemical_groups` | bool | `true` | Compute per-stage match rates for drug-relevant chemical groups |
| `chemical_group_names` | list[string] | `[rings_in_drugs, privileged_scaffolds]` | Chemical group sets to evaluate |
