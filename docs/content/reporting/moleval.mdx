...
| **Filters** | Fraction passing vendor MCF + PAINS CSVs shipped with MolEval | 0--1 | Higher = cleaner set under those catalogs. This is **not** Stage 3 Common Alerts PAINS (`rd_filters` / `common_alerts_collection.csv`). |
...
| **Descriptors** | `stages/02_descriptors_initial/filtered_molecules.csv` | After descriptor calculation and initial property filtering |
...
| `filters` | bool | `true` | MCF + PAINS passage rate from vendor files `src/hedgehog/vendor/moleval/metrics/mcf.csv` and `wehi_pains.csv` |

There are no `pains_file_path` / `mcf_file_path` config keys.
Paths are fixed next to the vendored MolEval metrics code.
Stage 3 structural PAINS uses a different catalog and enforcement path.
...
documented seed parameter for reproducible subsampling.
