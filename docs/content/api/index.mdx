---
title: API Overview
timestamp: 1789069744383
---

# API Overview

Generated automatically at build time by `scripts/generate_api_docs.py` (2026-09-10 19:49:04 UTC).
...
| Parameter | Kind | Flags | Type | Required | Default | Description |
| --- | --- | --- | --- | --- | --- | --- |
| `config_path` | option | `--config, -c` | `STRING` | no | `None` | Master YAML config path. Defaults to the packaged config, or to RUN_FOLDER/configs/master_config_resolved.yml with --continue. |
| `generated_mols_path` | option | `--mols, -m` | `STRING` | no | `None` | Molecule file, directory, or glob. Shell-expanded SDF/CSV lists after --mols are accepted (overrides config). |
| `out_dir` | option | `--out, -o` | `STRING` | no | `None` | Output naming base; creates a fresh numbered folder. |
...
| `continue_folder` | option | `--continue` | `STRING` | no | `None` | Continue an unfinished run from its first incomplete stage. |
| `force_new_folder` | option | `--force-new` | `BOOL` | no | `False` | Explicitly request the default fresh-folder behavior. |
...
| `evaluate_docked_coordinates` | option | `--evaluate-docked-coordinates` | `BOOL` | no | `False` | For SDF inputs, also apply 3D filters to docked coordinates; requires enabled docking with at least one tool. |
| `align_config_percentile` | option | `--align-config` | `<FloatRange 1e-06<=x<=100.0>` | no | `None` | Align numeric thresholds to cover this percentage of the target_mols_path molecules; structural policy remains unchanged. |
