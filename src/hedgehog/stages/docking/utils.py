...
def _create_docking_config_file(cfg, ligands_dir, receptor, ligands_path, output_sdf, config_path, tool_name):
...
def _create_smina_config_file(cfg, ligands_dir, receptor, ligands_path, config_path, output_sdf):
    ...
    return _create_docking_config_file(cfg, ligands_dir, receptor, ligands_path, output_sdf, config_path, 'smina')
...
def _create_gnina_config_file(cfg, ligands_dir, receptor, ligands_path, output_sdf, config_path):
    ...
    return _create_docking_config_file(cfg, ligands_dir, receptor, ligands_path, output_sdf, config_path, 'gnina')
...
def _write_protein_prep_bash(f, protein_prep_cmd, ligands_dir, tool_name):
    ...
    protein_output_file, protein_output_abs_path = _extract_pdb_output_from_cmd(protein_prep_cmd, ligands_dir)
    ...
    f.write('  echo "WARNING: Protein preparation command exited with code $PREP_EXIT_CODE"\n')
    f.write('fi\n')
...
def _write_receptor_check_bash(f, receptor):
    ...
    f.write('# Final check - wait a moment and verify file still exists and is readable\n')
    f.write('sleep 1\n')
...
def _write_docking_command_bash(f, ligands_dir, docking_bin, config_file, tool_name):
    ...
    f.write(f'cd {ligands_dir}\n')
    f.write(f'echo "Starting {tool_name.upper()} docking with config: {config_file.name}"\n')
...
def _get_receptor_and_prep_cmd(cfg, ligands_dir, protein_preparation_tool, tool_name):
    ...
    if not receptor_path:
        logger.error(f'{tool_name.upper()}: Receptor file not found: {original_receptor}')
        return None, None
    ...
    prepared_receptor, protein_prep_cmd = _prepare_protein_for_docking(receptor, ligands_dir, protein_preparation_tool)
...
def _setup_smina(cfg, ligands_dir, ligands_csv, protein_preparation_tool, base_folder=None):
    ...
    try:
        ...
        receptor, protein_prep_cmd = _get_receptor_and_prep_cmd(cfg, ligands_dir, protein_preparation_tool, 'smina')
        if not receptor:
            return None
...
def _setup_gnina(cfg, base_folder, ligands_dir, ligands_csv, ligand_preparation_tool, protein_preparation_tool):
    ...
    try:
        ...
        receptor, protein_prep_cmd = _get_receptor_and_prep_cmd(cfg, ligands_dir, protein_preparation_tool, 'gnina')
        if not receptor:
            return None
