...
class ValidationHandler:
    ...
    def validate_input_file(self, path: str) -> dict[str, Any]:
        ...
        if size > 1024 * 1024 * 1024:  # 1GB
            result['warnings'].append('File is very large (>1GB), processing may be slow')
    ...
    def validate_receptor_pdb(self, path: str) -> dict[str, Any]:
        ...
        if file_path.suffix.lower() != '.pdb':
            result['warnings'].append(f'Expected .pdb extension, got: {file_path.suffix}')
        ...
        try:
            ...
            if atom_count == 0:
                result['warnings'].append('No protein atoms found (only HETATM records)')
    ...
    def validate_output_directory(self, path: str) -> dict[str, Any]:
        ...
        if dir_path.exists():
            ...
            if any(dir_path.iterdir()):
                result['warnings'].append('Output directory is not empty, files may be overwritten')
        else:
    ...
    def validate_config(self, config_type: str, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if config_type == 'main':
            ...
            if config.get('run', False):
                if not config.get('receptor_pdb'):
                    result['errors'].append('Receptor PDB is required when docking is enabled')
