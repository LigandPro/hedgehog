...
def preprocess_input_with_rdkit(input_path, folder_to_save, logger):
    ...
    try:
        ...
        output_df = pl.DataFrame(cleaned_data)

        initial_count = len(output_df)
        output_df = output_df.unique(subset=['smiles', 'model_name'], keep='first')
        ...
        output_df.write_csv(prepared_output)
