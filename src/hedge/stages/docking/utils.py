    ...
    with open(script_path, "w") as f:
        ...
        if protein_prep_cmd:
            ...
            if protein_output_abs_path:
                ...
                f.write("  exit 1\n")
                f.write("else\n")
                f.write('  echo "Protein preparation completed successfully"\n')
                f.write("fi\n")
    ...
    with open(script_path, "w") as f:
        ...
        if protein_preparation_cmd:
            ...
            if protein_output_file:
                ...
                f.write(f'  mv "{protein_output_file}" "{protein_output_abs_path}"\n')
                f.write("fi\n")
                f.write("# Verify prepared file exists and is not empty\n")
                f.write(f'if [ ! -f "{protein_output_abs_path}" ] && [ ! -f "{protein_output_file}" ]; then\n')
                f.write(f'  echo "ERROR: Protein preparation failed - output file not found at {protein_output_abs_path}"\n')
                f.write('  echo "Current directory: $(pwd)"\n')
                f.write('  echo "Listing files in current directory:"\n')
        ...
        f.write(f': > "{output_sdf!s}"\n')
        f.write('echo "Starting GNINA docking..."\n')
        f.write(" ".join(cmd) + "\n")
            ...
            if original_receptor_path.exists():
                ...
                if prepared_receptor_path != original_receptor and prep_cmd:
                    try:
                        import subprocess
                        import time
                        result = subprocess.run(prep_cmd, check=False, shell=True, capture_output=True, text=True, cwd=str(ligands_dir))
