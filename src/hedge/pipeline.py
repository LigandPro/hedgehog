"""Molecular analysis pipeline orchestration."""
...
class PipelineStage:
    ...
    def __init__(self, name: str, config_key: str, directory: str) -> None:
        """Initialize a pipeline stage.

        Args:
            name: Stage name identifier
            config_key: Configuration key for this stage
            directory: Output directory for this stage
        """
        self.name = name
...
class DataChecker:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        """Initialize data checker with configuration.
    ...
    def check_stage_data(self, stage_name: str) -> bool:
        ...
        Returns
        -------
            True if data exists and is not empty, False otherwise
        """
        try:
    ...
    def _get_stage_output_path(self, stage_name: str) -> Path | None:
        ...
        Returns
        -------
            Path to the stage output file, or None if stage is unknown
        """
        if stage_name == DIR_DESCRIPTORS:
            return (
                self.base_path
                / DIR_DESCRIPTORS
                / FILE_PASS_SMILES_TEMPLATE.format(stage=DIR_DESCRIPTORS)
            )
        if stage_name == DIR_STRUCT_FILTERS:
            return (
                self.base_path
                / DIR_STRUCT_FILTERS
                / FILE_PASS_SMILES_TEMPLATE.format(stage=DIR_STRUCT_FILTERS)
            )
        if stage_name == DIR_BEFORE_DESCRIPTORS:
            return (
                self.base_path
                / f"{DIR_BEFORE_DESCRIPTORS}_StructFilters"
                / FILE_PASS_SMILES_TEMPLATE.format(stage=DIR_STRUCT_FILTERS)
            )
...
class PipelineStageRunner:
    ...
    DATA_SOURCE_PRIORITY = [
        ...
        DIR_STRUCT_FILTERS,
        DIR_DESCRIPTORS,
        DIR_BEFORE_DESCRIPTORS,
    ]

    def __init__(self, config: dict[str, Any], data_checker: DataChecker) -> None:
        """Initialize pipeline stage runner.

        Args:
            config: Pipeline configuration dictionary
            data_checker: Data checker instance for validating stage outputs
        """
        self.config = config
    ...
    def find_latest_data_source(self) -> str | None:
        ...
        Returns
        -------
            Name of the latest stage with data, or None if no data found
        """
    ...
    def run_structural_filters(self, prefix: str) -> bool:
        ...
        Args:
            prefix: Stage prefix (e.g., 'beforeDescriptors' or 'Descriptors')
        ...
        """
        try:
            has_data = self.data_checker.check_stage_data(prefix)
            if prefix != DIR_BEFORE_DESCRIPTORS and not has_data:
                if self.config.get(OVERRIDE_SINGLE_STAGE) == STAGE_STRUCT_FILTERS:
                    ...
                    logger.warning(
                        f"No data for structural filters with prefix: {prefix}"
                    )
                    return False
            ...
            if not latest_data_source:
                if self.config.get(OVERRIDE_SINGLE_STAGE) == STAGE_SYNTHESIS:
                    ...
                    if sampled_path.exists():
                        ...
                        logger.warning(
                            "No data available for synthesis, check `config.yml` file"
                        )
                        return False
                else:
                    logger.warning(
                        "No data available for synthesis, check provided path"
                    )
                    return False
    ...
    def _parse_docking_tools(self, tools_cfg: Any) -> list[str]:
        ...
        Args:
            tools_cfg: Tools configuration (string, list, or other)

        Returns
        -------
            List of docking tool names
        """
    ...
    def docking_results_present(self) -> bool:
        ...
        Returns
        -------
            True if results are present for any configured tool, False otherwise
        """
        try:
    ...
    def _get_gnina_output_dir(
        ...
        Args:
            cfg: Docking configuration
            base_folder: Base output folder

        Returns
        -------
            Path to GNINA output directory
        """
    ...
    def _directory_has_files(self, dir_path: Path) -> bool:
        ...
        Returns
        -------
            True if directory has files, False otherwise
        """
        try:
...
class MolecularAnalysisPipeline:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        """Initialize the molecular analysis pipeline.
            ...
            try:
                ...
                if stage.name == STAGE_STRUCT_INI_FILTERS:
                    if OVERRIDE_STRUCT_FILTERS_BEFORE in self.config:
                        ...
                        run_before = stage_config.get(
                            CONFIG_RUN_BEFORE_DESCRIPTORS, False
                        )
                ...
                logger.debug(f"Stage {stage.name}: {status}")
            except Exception as e:
    ...
    def get_latest_data(
        ...
        Args:
            skip_descriptors: If True, skip Descriptors stage when looking
                for data source

        Returns
        -------
            DataFrame with latest molecular data, or None if no data found
        """
        ...
        if latest_source:
            try:
                ...
                logger.info(
                    f"Loaded latest data from {latest_source}: {len(data)} molecules"
                )

                if len(data) == 0:
                    logger.warning(
                        f"No molecules in {latest_source}, trying previous step..."
                    )
                    ...
                    for next_source in priority[current_idx + 1 :]:
                        if self.data_checker.check_stage_data(next_source):
                            ...
                            if len(data) > 0:
                                logger.info(
                                    f"Loaded data from {next_source}: "
                                    f"{len(data)} molecules (previous step)"
                                )
                                return data
    ...
    def run_pipeline(self, data: pd.DataFrame) -> bool:
        ...
        if self.stages[0].enabled:
            logger.info(
                "---------> [#B29EEE]Stage 1': "
                "Pre-descriptors Structural Filters[/#B29EEE]"
            )
        ...
        if self.stages[2].enabled:
            logger.info(
                "---------> [#B29EEE]Stage 2: "
                "Post-descriptors Structural Filters[/#B29EEE]"
            )
            ...
            if self.stage_runner.run_synthesis():
                ...
                if output_path.exists():
                    try:
                        ...
                        if len(df_check) == 0:
                            logger.info(
                                "No molecules left after synthesis; "
                                "ending pipeline early."
                            )
                            self._log_pipeline_summary()
                            initial_count = len(data)
                            final_data = self.get_latest_data(skip_descriptors=True)
                            ...
                            return success_count == total_enabled_stages
                        logger.warning(
                            "Synthesis stage failed but output file exists. "
        ...
        if self.stages[5].enabled:
            ...
            if final_data is not None and len(final_data) > 0:
                ...
                logger.info(
                    "No molecules available for final descriptors calculation "
                    "(skipping descriptors stage as source)"
                )
                if final_data is not None and len(final_data) == 0:
                    logger.info(
                        "Ending pipeline: no molecules from previous steps "
                        "(excluding descriptors)"
                    )

        logger.info(
            f"Pipeline completed: {success_count}/{total_enabled_stages} "
            "stages successful"
        )
        self._log_pipeline_summary()
        ...
        for stage in self.stages:
            if stage.enabled:
                if stage.completed:
                    ...
                    (
                        stage.name == STAGE_STRUCT_FILTERS
                        and not self.data_checker.check_stage_data(DIR_DESCRIPTORS)
                    )
                    or (
                        stage.name == STAGE_FINAL_DESCRIPTORS
                        and not self.data_checker.check_stage_data(DIR_STRUCT_FILTERS)
                    )
                    or (
                        stage.name == STAGE_SYNTHESIS
                        and not self.data_checker.check_stage_data(DIR_STRUCT_FILTERS)
                    )
                ):
...
def _save_config_snapshot(config: dict[str, Any]) -> None:
    ...
    try:
        ...
        with master_config_path.open("w") as f:
            yaml.safe_dump(config, f, sort_keys=False)

        config_keys = [k for k in config if k.startswith("config_")]
