...
def _write_smiles_smi_and_csv(df: pd.DataFrame, smi_path: Path | None, csv_path: Path) -> dict:
    ...
    rows = []
    for smi, name_val, mn, midx in zip(df[smiles_col].astype(str),
                                       names,
...
def run_docking(config: dict) -> bool:
    ...
    if 'gnina' in tools_list:
        ...
        if not ld_library_path:
            ...
            if env_path:
                ...
                if len(torch_libs) > 0 and (torch_libs[0] / 'libcudnn.so.9').exists():
    ...
    try:
        ...
        meta = {
            ...
            'tools_prepared': list(job_ids.keys()),
            ...
            if 'smina' in tools_list:
                ...
                if smina_script.exists():
                    ...
                    if background:
                        ...
                        logger.info('SMINA docking completed successfully')
                        if 'smina' in job_ids:
                            run_status['smina_job_id'] = job_ids['smina']
            ...
            if 'gnina' in tools_list:
                ...
                if gnina_script.exists():
                    ...
                    if background:
                        ...
                        logger.info('GNINA docking completed successfully')
                        if 'gnina' in job_ids:
                            run_status['gnina_job_id'] = job_ids['gnina']
                    cfg_out_dir = cfg.get('gnina_output_dir')
                    if cfg_out_dir:
