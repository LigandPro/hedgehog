...
def check_left_any_data(config, key_word):
    path = config['folder_to_save'] + f'leftMolsAfter{key_word}.csv'
...
def run_structural_filters(config, mode):
    if check_left_any_data(config, 'Descriptors'):
        ...
        if config_structFilters['run']:
            ...
            structural_filters_main(config, mode)
    else:
...
def calculate_metrics(data, config, mode='single_comparison'):
    run_descriptors(data, config)
    run_structural_filters(config, mode=mode)
    run_molscore_evaluation(config, mode=mode)
    # run_molscore_evaluation(config, mode=mode)
