...
class SybaClassifier:
    ...
    def fitDefaultScore(self):
        ...
        syba_file = os.path.join(resources_dir, "syba4.csv.gz")
        
        # Check if file exists directly
        if not os.path.exists(syba_file):
            # Try to reconstruct from parts
            import glob
            parts = sorted(glob.glob(os.path.join(resources_dir, "syba4.csv.gz.part_*")))
            if parts:
                # Combine parts into one file
                with open(syba_file, 'wb') as outfile:
                    for part in parts:
                        with open(part, 'rb') as infile:
                            outfile.write(infile.read())
        
        with gzip.open(syba_file, mode="rt") as counts:
